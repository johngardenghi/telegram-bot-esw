"""
Implementa os handlers dos comandos Telegram para indicação de professor orientador.
"""
...
GMAIL_USER = CredentialManager.read_credential("gmail_user")
GMAIL_PASSWORD = CredentialManager.read_credential("gmail_password")
...
# ------------------------------------------------
# INICIA A CONVERSA PARA A INDICAÇÃO DE ORIENTADOR
# ------------------------------------------------
async def inicia_conversa(update: Update, context: CallbackContext):
    ...
    orientador_service = context.application.bot_data["orientador_service"]
    solicitacao_estagio_service = context.application.bot_data["solicitacao_estagio_service"]

    telegram_id = update.message.from_user.id

    if not administracao_service.eh_admin(telegram_id):
        # Verifica se já há uma orientação de estágio indicada para o usuário que está chamando
        solicitacaoEstagio = solicitacao_estagio_service.verifica_solicitacao_ativa(telegram_id)
        ...
        if solicitacaoEstagio is not None:
            orientadorEstagio = orientador_service.get_orientador_by_id(solicitacaoEstagio.orientador)
        ...
        orientadorEstagio = orientador_service.indica_orientador()
...
# -------------------------------------------------------------------------
# VERIFICA SE TODOS OS FATORES BÁSICOS PARA INDICAR O ORIENTADOR SE CUMPREM
# -------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# ENCAMINHA AS INSTRUÇÕES PARA O ALUNO, JUNTO COM O NOME DO PROFESSOR ORIENTADOR
# ------------------------------------------------------------------------------
async def encaminhar_instrucoes(update: Update, context: CallbackContext) -> int:
    ...
    solicitacao_estagio_service = context.application.bot_data["solicitacao_estagio_service"]
    solicitacao_estagio = SolicitacaoEstagio(
        None,
        orientadorEstagio.id,
...
# ---------------------------------------------------------------------------------------------
# FUNÇÕES PARA OBTER A LISTA DE ORIENTADORES DISPONIVEIS, AFASTADOS E INATIVOS NA PRESENTE DATA
# ---------------------------------------------------------------------------------------------
async def lista_orientadores(update: Update, context: CallbackContext) -> None:
    ...
    if administracao_service.eh_admin(update.message.from_user.id):
        result = await orientador_service.disponiveis()
        for msg in result:
...
async def lista_orientadores_afastados(update: Update, context: CallbackContext) -> None:
    ...
    orientador_service = context.application.bot_data["orientador_service"]
    if administracao_service.eh_admin(update.message.from_user.id):
        result = await orientador_service.afastados()
        for msg in result:
            await update.message.reply_text(msg)
    else:
        await update.message.reply_text("Você não tem privilégios para executar esta atualização.")
...
async def lista_orientadores_inativos(update: Update, context: CallbackContext) -> None:
    ...
    orientador_service = context.application.bot_data["orientador_service"]
    if administracao_service.eh_admin(update.message.from_user.id):
        result = await orientador_service.inativos()
        for msg in result:
            await update.message.reply_text(msg)
    else:
        await update.message.reply_text("Você não tem privilégios para executar esta atualização.")
...
# -------------------------------
# RETORNA OS HANDLERS PARA A MAIN
# -------------------------------
def get_orientador_estagio_handler() -> ConversationHandler:
...
def get_lista_orientadores_disponiveis_handler() -> CommandHandler:
    return CommandHandler("orientadores_disponiveis", lista_orientadores)

def get_lista_orientadores_afastados_handler() -> CommandHandler:
    return CommandHandler("orientadores_afastados", lista_orientadores_afastados)

def get_lista_orientadores_inativos_handler() -> CommandHandler:
    return CommandHandler("orientadores_inativos", lista_orientadores_inativos)
